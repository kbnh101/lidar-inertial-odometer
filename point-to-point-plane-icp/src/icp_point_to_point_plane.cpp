...
void IcpPointToPointPlane::set_target(const common::PointCloud& target)
{
    target_ = target;
...
std::vector<IcpPointToPointPlane::Correspondence> IcpPointToPointPlane::find_correspondences(const Eigen::Isometry3d& pose) const
{
...
void IcpPointToPointPlane::evaluate_error(const Eigen::Isometry3d& pose, const std::vector<Correspondence>& correspondences, double* rms, double* mean_abs,
                                          double* max_abs) const
{
    ...
    for (const Correspondence& correspondence : correspondences)
    {
        ...
        // Combined weighted norm, before the independently applied robust losses.
        const Eigen::Vector3d error = transformed - target_[correspondence.target_index].point;
        const double plane_error = target_[correspondence.target_index].normal.dot(error);
        const double residual = std::sqrt(options_.point_weight * error.squaredNorm() + options_.plane_weight * plane_error * plane_error);
...
IcpResult IcpPointToPointPlane::do_icp(const Eigen::Isometry3d& initial_guess)
{
    ...
    if (target_.empty())
    {
        throw std::runtime_error("IcpPointToPointPlane::do_icp: target cloud is empty");
    }
...
IcpResult IcpPointToPointPlane::do_icp(const common::PointCloud& source, const common::PointCloud& target, const Eigen::Isometry3d& initial_guess)
{
    set_source(source);
    set_target(target);
