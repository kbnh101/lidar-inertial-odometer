...
void print_pose(const Eigen::Isometry3d& pose)
{
    ...
    const Eigen::Vector3d euler = p2p_icp::rotation_to_euler_zyx(pose.linear());
...
int main(int argc, char** argv)
{
    ...
    try
    {
        const p2p_icp::PointCloud source_points = p2p_icp::load_point_cloud(source_path);
        const p2p_icp::PointCloud target_points = p2p_icp::load_point_cloud(target_path);
