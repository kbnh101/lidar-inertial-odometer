...
void LioOdometer::ProcessScan(const QueuedScan& scan)
{
    ...
    // KITTI clouds carry no normals, yet point-to-point-plane ICP absolutely needs target normals.
    ...
    // --- 4. scan-to-map point-to-point-plane ICP ---------------------------------------
    ...
    if (!local_map_.empty() && static_cast<int>(features.planar.size()) >= options_.min_icp_correspondences)
    {
        ...
        const p2ptpl_icp::IcpResult icp_result = icp_.do_icp(lidar_pose_prediction);
